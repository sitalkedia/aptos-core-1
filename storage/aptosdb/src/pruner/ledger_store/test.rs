    ...
    for (epoch, version) in &epoch_to_latest_version {
        ...
        for (inner_epoch, inner_version) in &epoch_to_latest_version {
            if inner_epoch < epoch {
                // Epoch is pruned, so we can not read the ledger info schema and epoch by version
                // from the DB.
                assert!(aptos_db
                    .db
                ...
                // Epoch is not pruned, so we can read the ledger info schema and epoch by version
                // from the DB.
                assert!(aptos_db
                    .db
                    .get::<LedgerInfoSchema>(inner_epoch)
